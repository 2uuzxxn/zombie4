...
class Player {
  ...
  draw(p) {
    if (!this.alive) return;
    const tailCol = this.steelTailTimer > 0 ? '#78909C' : this.displayColor;
    p.noStroke();
    ...
    if (this.boostTimer > 0) {
      p.fill(0, 255, 200, 50); p.noStroke();
      p.rect(x-4, y-4, TILE_SIZE+8, TILE_SIZE+8, 6);
    }
    if (this.bombFlash > 0 && Math.floor(p.frameCount/3) % 2 === 0) {
      p.fill(255, 210, 0, 110); p.noStroke();
      p.rect(x-4, y-4, TILE_SIZE+8, TILE_SIZE+8, 6);
    }
    p.fill(p.red(p.color(this.displayColor)), p.green(p.color(this.displayColor)), p.blue(p.color(this.displayColor)), 60);
    p.rect(x-1, y-1, TILE_SIZE+2, TILE_SIZE+2, 6);
    p.fill(this.displayColor); p.noStroke();
    p.rect(x+1, y+1, TILE_SIZE-2, TILE_SIZE-2, 4);
    p.fill(255, 255, 255, 220); p.textAlign(p.CENTER, p.CENTER); p.textSize(9);
    p.textFont('Share Tech Mono');
    p.text(this.id, x+TILE_SIZE/2, y+TILE_SIZE/2);
    if (this.steelTailTimer > 0) {
      p.fill(176, 190, 197, 220); p.textSize(6);
